# RxApps360 - Roles & Permissions Test
...
4. Navigate to the **Roles & Permissions** page.
5. Verify that the Roles & Permissions page is displayed.
...
```ts
rxasset.admin.roles-permissions.test.ts
```
...
### Step 4: Open Roles & Permissions Page

```ts
const assetPage = createRxAssetPage(page);

await assetPage.enterRolesPermissionsView();
```

#### Description

* Navigates to the Roles & Permissions section under RxAsset Administration.
* Loads the user role and permission management page.

---

### Step 5: Validate Roles & Permissions Page
...
* Roles & Permissions page loads successfully.
* Roles & Permissions heading is visible.
...
```ts
await assetPage.screenshot("rxasset-roles-permissions");
```

#### Screenshot

```text
rxasset-roles-permissions.png
```
...
```bash
npx playwright test rxasset.admin.roles-permissions.test.ts
```
...
* Opening the Roles & Permissions page.
* Screenshot capture.
