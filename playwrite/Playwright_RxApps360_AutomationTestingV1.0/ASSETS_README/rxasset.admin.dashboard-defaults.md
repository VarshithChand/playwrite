# RxApps360 - Dashboard Defaults Test

## Overview

This Playwright automation test verifies that a user can:

1. Log in to RxApps360 using username, password, and MFA (TOTP).
2. Successfully access the application dashboard.
3. Open the RxAsset™ module.
4. Navigate to the **Dashboard Defaults** page.
5. Verify that the Dashboard Defaults page is displayed.
6. Capture a screenshot for test evidence.

---

## Test File

```ts
rxasset.admin.dashboard-defaults.test.ts
```

---

## Prerequisites

### Required Software

* Node.js (v18 or later)
* npm
* Playwright

### Install Dependencies

```bash
npm install
npx playwright install
```

---

## Environment Configuration

Create a `.env` file in the project root and configure the following variables:

```env
APP_USER=your_username
APP_PASSWORD=your_password
MFA_SECRET=your_totp_secret
```

---

## Test Flow

### Step 1: Login with MFA

```ts
const loginPage = createRxLoginPage(page);

await loginPage.navigate();

await loginPage.loginWithMfa(
  ENV_CONFIG.appUser,
  ENV_CONFIG.appPassword,
  ENV_CONFIG.mfaSecret
);
```

### Description

* Opens the RxApps360 login page.
* Enters username and password.
* Generates a TOTP code using the MFA secret.
* Completes authentication.

---

### Step 2: Verify Successful Login

```ts
const home = createRxHomePage(page);

expect(await home.isAvatarVisible()).toBe(true);
```

### Description

* Verifies that the user avatar is visible.
* Confirms successful login and dashboard access.

---

### Step 3: Open RxAsset™

```ts
await home.openRxAsset();
```

### Description

* Navigates from the application dashboard to the RxAsset™ module.

---

### Step 4: Open Dashboard Defaults Page

```ts
const assetPage = createRxAssetPage(page);

await assetPage.enterDashboardDefaultsView();
```

### Description

* Opens the Dashboard Defaults section within RxAsset Administration.
* Loads default dashboard configuration settings.

---

### Step 5: Validate Dashboard Defaults Page

```ts
await expect(
  page.locator("main").getByRole("heading", {
    name: /Dashboard Defaults/i,
  })
).toBeVisible({
  timeout: 30000,
});
```

### Validation

Verifies that:

* Dashboard Defaults page loads successfully.
* Dashboard Defaults heading is displayed.
* User is navigated to the correct page.

---

### Step 6: Capture Screenshot

```ts
await assetPage.screenshot("rxasset-dashboard-defaults");
```

### Screenshot

```text
rxasset-dashboard-defaults.png
```

---

## Running the Test

Run all tests:

```bash
npx playwright test
```

Run only this test:

```bash
npx playwright test rxasset.admin.dashboard-defaults.test.ts
```

Run in headed mode:

```bash
npx playwright test --headed
```

Run using Playwright UI:

```bash
npx playwright test --ui
```

---

## Expected Result

✅ User successfully logs in using MFA.

✅ RxAsset™ module opens successfully.

✅ Dashboard Defaults page loads successfully.

✅ Dashboard Defaults heading is visible.

✅ Screenshot is captured successfully.

---

## Framework Design

```text
src/
├── pages/
│   ├── RxLoginPage.ts
│   ├── RxHomePage.ts
│   └── RxAssetPage.ts
│
├── tests/
│   └── rxapps/
│       └── rxasset.admin.dashboard-defaults.test.ts
│
└── config/
    └── env.config.ts
```

---

## Notes

* The test automatically skips if required environment variables are missing.
* MFA authentication uses a TOTP secret from the `.env` file.
* Screenshots are captured for execution evidence and debugging.
* Uses the Page Object Model (POM) design pattern for maintainability and reusability.
