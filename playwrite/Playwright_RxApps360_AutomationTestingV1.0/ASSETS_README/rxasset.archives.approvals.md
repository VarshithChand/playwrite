# RxApps360 - Archive Approvals Test
...
4. Navigate to the **Archive Approvals** page.
5. Verify that the Archive Approvals page is displayed.
...
```ts
rxasset.archives.approvals.test.ts
```
...
### Step 4: Open Archive Approvals Page

```ts
const assetPage = createRxAssetPage(page);

await assetPage.enterArchiveApprovalsView();
```

#### Description

* Navigates to the Archive Approvals section under RxAsset Archives.
* Loads the archive approval records page.

---

### Step 5: Validate Archive Approvals Page

```ts
await expect(
  page.locator("main").getByRole("heading", {
    name: /Archive Approvals/i,
  })
).toBeVisible({
  timeout: 30000,
});
...
* Archive Approvals page loads successfully.
* Archive Approvals heading is visible.
...
```ts
await assetPage.screenshot("rxasset-archive-approvals");
```

#### Screenshot

```text
rxasset-archive-approvals.png
```
...
```bash
npx playwright test rxasset.archives.approvals.test.ts
```
...
* Archives navigation.
* Opening the Archive Approvals page.
* Screenshot capture.
