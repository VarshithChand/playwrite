# RxApps360 - Archived Assets Test
...
4. Navigate to the **Archived Assets** page.
5. Verify that the Archived Assets page is displayed.
...
```ts
rxasset.archives.assets.test.ts
```
...
### Step 4: Open Archived Assets Page

```ts
const assetPage = createRxAssetPage(page);

await assetPage.enterArchivedAssetsView();
```

#### Description

* Navigates to the Archived Assets section under RxAsset Archives.
* Loads the archived assets records page.

---

### Step 5: Validate Archived Assets Page

```ts
await expect(
  page.locator("main").getByRole("heading", {
    name: /Archived Assets/i,
  })
).toBeVisible({
  timeout: 30000,
});
...
* Archived Assets page loads successfully.
* Archived Assets heading is visible.
...
```ts
await assetPage.screenshot("rxasset-archived-assets");
```

#### Screenshot

```text
rxasset-archived-assets.png
```
...
```bash
npx playwright test rxasset.archives.assets.test.ts
```
...
* Archives navigation.
* Opening the Archived Assets page.
* Screenshot capture.
