...
test.describe("RxApps360 - RxAsset Reports", () => {

  test("user can log in with MFA, open RxAsset, and open Work Order Reports", async ({ page }) => {
    test.skip(
    ...
    await assetPage.enterWorkOrderReportsView();

    await expect(page.locator("main").getByRole("heading", { name: /Work Order Reports/i })).toBeVisible({
      timeout: 30_000,
    });

    await assetPage.screenshot("rxasset-work-order-reports");
  });
