...
test.describe("RxApps360 - RxAsset Work Orders", () => {
  ...
  test("user can log in with MFA, open RxAsset, and open Create Work Order", async ({ page }) => {
    test.skip(
    ...
    await assetPage.enterCreateWorkOrderView();

    await expect(page.locator("main").getByRole("heading", { name: /Create.*Work Order/i })).toBeVisible({
      timeout: 30_000,
    });

    await assetPage.screenshot("rxasset-create-work-order");
  });
