...
test.describe("RxApps360 - RxAsset Administration", () => {
  ...
  test("user can log in with MFA, open RxAsset, and open Rate Configuration", async ({ page }) => {
    test.skip(
    ...
    await expect(page.locator("main").getByRole("heading", { name: /Rate Configuration/i })).toBeVisible({
      timeout: 30_000,
    });
