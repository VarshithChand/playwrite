...
test.describe("RxApps360 - RxAsset View Assets", () => {

  test("user can log in with MFA, open RxAsset, and open View Assets", async ({ page }) => {
    test.skip(
    ...
    await expect(page.locator("main").getByRole("heading", { name: /^(Assets|View Assets)$/i })).toBeVisible({
      timeout: 30_000,
    });
    await expect(page.locator("main").getByText("Filter Assets", { exact: true })).toBeVisible({
      timeout: 30_000,
    });
    await expect(page.locator("main").getByRole("button", { name: /^Search$/i })).toBeVisible({
      timeout: 30_000,
    });
