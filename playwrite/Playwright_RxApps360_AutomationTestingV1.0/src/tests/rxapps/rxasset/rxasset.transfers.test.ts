...
test.describe("RxApps360 - RxAsset Transfers", () => {

  test("user can log in with MFA, open RxAsset, and open Transfers", async ({ page }) => {
    test.skip(
    ...
    await assetPage.enterTransfersView();

    await expect(page.locator("main").getByRole("heading", { name: /Transfers/i })).toBeVisible({
      timeout: 30_000,
    });

    await assetPage.screenshot("rxasset-transfers");
  });
