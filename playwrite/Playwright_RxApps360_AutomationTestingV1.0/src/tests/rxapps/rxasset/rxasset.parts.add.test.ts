...
test.describe("RxApps360 - RxAsset Parts", () => {
  ...
  test("user can log in with MFA, open RxAsset, and open Add Parts", async ({ page }) => {
    test.skip(
    ...
    await assetPage.enterAddPartsView();

    await expect(page.locator("main").getByRole("heading", { name: /Add.*Parts?/i })).toBeVisible({
      timeout: 30_000,
    });

    await assetPage.screenshot("rxasset-add-parts");
  });
