...
test.describe("RxApps360 - RxAsset Parts", () => {

  test("user can log in with MFA, open RxAsset, and open View Parts", async ({ page }) => {
    test.skip(
    ...
    await assetPage.enterViewPartsView();

    await expect(page.locator("main").getByRole("heading", { name: /Parts|View Parts/i })).toBeVisible({
      timeout: 30_000,
    });

    await assetPage.screenshot("rxasset-view-parts");
  });
