...
test.describe("RxApps360 - RxAsset Reports", () => {
  ...
  test("user can log in with MFA, open RxAsset, and open Cost Reports", async ({ page }) => {
    test.skip(
    ...
    await assetPage.enterCostReportsView();

    await expect(page.locator("main").getByRole("heading", { name: /Cost Reports/i })).toBeVisible({
      timeout: 30_000,
    });

    await assetPage.screenshot("rxasset-cost-reports");
  });
