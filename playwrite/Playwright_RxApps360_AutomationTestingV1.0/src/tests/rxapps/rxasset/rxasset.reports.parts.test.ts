...
test.describe("RxApps360 - RxAsset Reports", () => {
  ...
  test("user can log in with MFA, open RxAsset, and open Parts Reports", async ({ page }) => {
    test.skip(
    ...
    await assetPage.enterPartsReportsView();

    await expect(page.locator("main").getByRole("heading", { name: /Parts Reports/i })).toBeVisible({
      timeout: 30_000,
    });

    await assetPage.screenshot("rxasset-parts-reports");
  });
