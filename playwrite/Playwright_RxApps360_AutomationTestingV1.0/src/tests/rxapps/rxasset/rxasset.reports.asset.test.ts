...
test.describe("RxApps360 - RxAsset Reports", () => {
  ...
  test("user can log in with MFA, open RxAsset, and open Asset Reports", async ({ page }) => {
    test.skip(
    ...
    await assetPage.enterAssetReportsView();

    await expect(page.locator("main").getByRole("heading", { name: /Asset Reports/i })).toBeVisible({
      timeout: 30_000,
    });

    await assetPage.screenshot("rxasset-asset-reports");
  });
